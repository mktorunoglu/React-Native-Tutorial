import {MyLocalizations} from '../enums/Localizations';
import {MyLocalizationTextKeys} from '../enums/LocalizationTextKeys';

export const MyLocalizationTexts = {
  [MyLocalizationTextKeys.Files]: {
    [MyLocalizations.English]: 'Files',
    [MyLocalizations.Turkish]: 'Dosyalar',
  },
  [MyLocalizationTextKeys.WithUsers]: {
    [MyLocalizations.English]: 'With Users',
    [MyLocalizations.Turkish]: 'Kullanıcılarla',
  },
  [MyLocalizationTextKeys.SharedLinks]: {
    [MyLocalizations.English]: 'Shared Links',
    [MyLocalizations.Turkish]: 'Paylaşım Bağlantıları',
  },
  [MyLocalizationTextKeys.WithYourGroups]: {
    [MyLocalizations.English]: 'With Your Groups',
    [MyLocalizations.Turkish]: 'Gruplarınızla',
  },
  [MyLocalizationTextKeys.WithYou]: {
    [MyLocalizations.English]: 'With You',
    [MyLocalizations.Turkish]: 'Sizinle',
  },
  [MyLocalizationTextKeys.Total]: {
    [MyLocalizations.English]: 'Total',
    [MyLocalizations.Turkish]: 'Toplam',
  },
  [MyLocalizationTextKeys.Usage]: {
    [MyLocalizations.English]: 'Usage',
    [MyLocalizations.Turkish]: 'Kullanım',
  },
  [MyLocalizationTextKeys.SharedByYou]: {
    [MyLocalizations.English]: 'Shared By You',
    [MyLocalizations.Turkish]: 'Paylaştıklarınız',
  },
  [MyLocalizationTextKeys.SharedWithYou]: {
    [MyLocalizations.English]: 'Shared With You',
    [MyLocalizations.Turkish]: 'Sizinle Paylaşılanlar',
  },
  [MyLocalizationTextKeys.MyUsage]: {
    [MyLocalizations.English]: 'My Usage',
    [MyLocalizations.Turkish]: 'Kullanımım',
  },
  [MyLocalizationTextKeys.Search]: {
    [MyLocalizations.English]: 'Search',
    [MyLocalizations.Turkish]: 'Ara',
  },
  [MyLocalizationTextKeys.AnErrorOccurred]: {
    [MyLocalizations.English]: 'An error occurred',
    [MyLocalizations.Turkish]: 'Bir hata oluştu',
  },
  [MyLocalizationTextKeys.User]: {
    [MyLocalizations.English]: 'User',
    [MyLocalizations.Turkish]: 'Kullanıcı',
  },
  [MyLocalizationTextKeys.Messages]: {
    [MyLocalizations.English]: 'Messages',
    [MyLocalizations.Turkish]: 'Mesajlar',
  },
  [MyLocalizationTextKeys.Notifications]: {
    [MyLocalizations.English]: 'Notifications',
    [MyLocalizations.Turkish]: 'Bildirimler',
  },
  [MyLocalizationTextKeys.Actions]: {
    [MyLocalizations.English]: 'Actions',
    [MyLocalizations.Turkish]: 'Eylemler',
  },
  [MyLocalizationTextKeys.Operations]: {
    [MyLocalizations.English]: 'Operations',
    [MyLocalizations.Turkish]: 'İşlemler',
  },
  [MyLocalizationTextKeys.Profile]: {
    [MyLocalizations.English]: 'Profile',
    [MyLocalizations.Turkish]: 'Profil',
  },
  [MyLocalizationTextKeys.Favorites]: {
    [MyLocalizations.English]: 'Favorites',
    [MyLocalizations.Turkish]: 'Favoriler',
  },
  [MyLocalizationTextKeys.Sharing]: {
    [MyLocalizations.English]: 'Sharing',
    [MyLocalizations.Turkish]: 'Paylaşım',
  },
  [MyLocalizationTextKeys.Repos]: {
    [MyLocalizations.English]: 'Repos',
    [MyLocalizations.Turkish]: 'Depolar',
  },
  [MyLocalizationTextKeys.Home]: {
    [MyLocalizations.English]: 'Home',
    [MyLocalizations.Turkish]: 'Ana Sayfa',
  },
  [MyLocalizationTextKeys.CheckLoginInformation]: {
    [MyLocalizations.English]:
      'Please check your login information and try again.',
    [MyLocalizations.Turkish]:
      'Lütfen giriş bilgilerinizi kontrol edip tekrar deneyiniz.',
  },
  [MyLocalizationTextKeys.Okey]: {
    [MyLocalizations.English]: 'Okey',
    [MyLocalizations.Turkish]: 'Tamam',
  },
  [MyLocalizationTextKeys.ChangeLanguage]: {
    [MyLocalizations.English]: 'Change Language',
    [MyLocalizations.Turkish]: 'Dili Değiştir',
  },
  [MyLocalizationTextKeys.Turkish]: {
    [MyLocalizations.English]: 'Turkish',
    [MyLocalizations.Turkish]: 'Türkçe',
  },
  [MyLocalizationTextKeys.English]: {
    [MyLocalizations.English]: 'English',
    [MyLocalizations.Turkish]: 'İngilizce',
  },
  [MyLocalizationTextKeys.Options]: {
    [MyLocalizations.English]: 'Options',
    [MyLocalizations.Turkish]: 'Seçenekler',
  },
  [MyLocalizationTextKeys.Logout]: {
    [MyLocalizations.English]: 'Logout',
    [MyLocalizations.Turkish]: 'Çıkış Yap',
  },
  [MyLocalizationTextKeys.Login]: {
    [MyLocalizations.English]: 'Login',
    [MyLocalizations.Turkish]: 'Giriş Yap',
  },
  [MyLocalizationTextKeys.Password]: {
    [MyLocalizations.English]: 'Password',
    [MyLocalizations.Turkish]: 'Şifre',
  },
  [MyLocalizationTextKeys.UserId]: {
    [MyLocalizations.English]: 'User ID',
    [MyLocalizations.Turkish]: 'Kullanıcı Kimliği',
  },
  [MyLocalizationTextKeys.ServerAddress]: {
    [MyLocalizations.English]: 'Server Address',
    [MyLocalizations.Turkish]: 'Sunucu Adresi',
  },
  [MyLocalizationTextKeys.AppCopyrightDescription]: {
    [MyLocalizations.English]: 'Copyright © 2023, All Rights Reserved',
    [MyLocalizations.Turkish]: 'Copyright © 2023, Tüm Hakları Saklıdır',
  },
  [MyLocalizationTextKeys.AppNameFull]: {
    [MyLocalizations.English]: 'Corporate File Sharing Platform',
    [MyLocalizations.Turkish]: 'Kurumsal Dosya Paylaşım Platformu',
  },
};
